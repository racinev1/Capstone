...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  1 12:24:54 2021
...
def generate_metrics_table(benchmarks, dataloader, metrics, PVM, device=None, dtype=None):
    ...
    PVM : dict
        Keys are banchmark names and values are pd.DataFrame
    ...
    for batch_ix, (X_batch, y_batch, timestamps, _) in enumerate(dataloader):
        ...
        for bm_name, bm in benchmarks.items():
            prev_w = torch.FloatTensor(PVM[bm_name].loc[timestamps].values)
            weights = bm(X_batch, prev_w)
            #update PVM
            w = weights.detach().cpu().numpy()
            if len(w.shape) == 2: # (batch_size, n_assets)
                PVM[bm_name].loc[np.asarray(timestamps) + 1] = w
            else:
                PVM[bm_name].loc[np.asarray(timestamps) + 1] = w[:, 0, :]
...
def plot_weights_density(weights):
    fig = go.Figure()
...
def generate_weights_table(network, dataloader, device=None, dtype=None):
    ...
    for X_batch, _, timestamps, _ in dataloader:
        ...
        if len(weights_batch.size) == 3:
            weights_batch = weights_batch[:, 0, :]
