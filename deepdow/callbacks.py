...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  1 12:55:04 2021
            ...
            for dl_name, dl in self.run.val_dataloaders.items():
                for bm_name, bm in self.run.models.items():
                    ...
                    for batch, (X_batch, y_batch, timestamps_batch, _) in enumerate(dl):
                        ...
                        for lb in lookbacks:
                            #####
                            batch_size, _, _, n_assets = X_batch.shape
                            prev_w = torch.ones((batch_size, n_assets))/n_assets
                            #####

                            weights = bm(X_batch[:, :, -lb:, :], prev_w)
                ...
                for dl_name, dl in self.run.val_dataloaders.items():
                    for batch, (X_batch, y_batch, timestamps_batch, _) in enumerate(dl):
                        ...
                        for lb in lookbacks:
                            ...
                            prev_w = torch.ones((batch_size, n_assets))/n_assets
                            ####
                            weights = model(X_batch[:, :, -lb:, :], prev_w)
