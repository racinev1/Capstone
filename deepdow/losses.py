...
def portfolio_returns(weights, y, input_type='log', output_type='simple', rebalance=False):
    ...
    weights : torch.Tensor
        Tensor of shape (n_samples, horizon, n_assets) representing the simple buy and hold strategy over the horizon.
    y : torch.Tensor
    ...
    if len(weights.shape) == 2: #weights is of shape (n_samples, n_assets)
        weights = weights.view(n_samples, 1, n_assets).repeat(1, horizon, 1)  # (n_samples, horizon, n_assets)
    
    out = (simple_returns * weights).sum(-1)
