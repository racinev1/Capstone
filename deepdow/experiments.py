...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  1 12:15:38 2021
...
class Run:
    ...
    PVM: pd.DataFrame
        Portfolio Vector Memory. Index are timestamps and columns are stocks.
    ...
    def __init__(self, network, loss, train_dataloader, PVM, val_dataloaders=None, metrics=None,
        ...
        try:
            ...
            for _ in range(n_epochs):
                ...
                for batch_ix, (X_batch, y_batch, timestamps, asset_names) in enumerate(self.train_dataloader):
                    ...
                    #get previous w
                    prev_w = torch.FloatTensor(self.PVM.loc[timestamps, asset_names].values)
                    ...
                    # Forward & Backward
                    weights = self.network(X_batch, prev_w)
                    ...
                    self.PVM.loc[np.asarray(timestamps) + 1, asset_names] = actions
